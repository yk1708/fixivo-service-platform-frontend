...
// Featured workers are fetched from the backend API
...
export function HomePage() {
  ...
  useEffect(() => {
    getVerifiedProviders()
      .then(providers => {
        const workers = providers.slice(0, 4).map((p, i) => ({
          ...providerToWorker(p),
  ...
  const handleSendRequest = (workerId: string) => {
    ...
    const worker = featuredWorkers.find(w => w.id === workerId);
    setRequestModal({
      open: true,
      workerId,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {loadingProviders ? (
              ...
              <div className="col-span-full text-center py-12">
                <div className="text-gray-400 text-sm">No professionals available yet.</div>
              </div>
            )}
          </div>
        </div>
