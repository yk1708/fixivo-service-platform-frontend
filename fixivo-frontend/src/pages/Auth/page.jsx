...
function LoginPage() {
  ...
  const handleGoogleCredentialResponse = (response) => {
    ...
    const activeRole = loginType === 'customer' ? 'customer' : 'provider';
    
    if (activeRole === 'provider' && isSignup && !formData.serviceType) {
      showToast('Please select your profession (Service Type) first before signing up with Google.', 'error');
      return;
    }
  ...
  if (buttonDiv) {
    ...
    window.google.accounts.id.renderButton(
      buttonDiv,
      {
        theme: "outline",
        size: "large",
        width: buttonDiv.clientWidth || 380,
        text: isSignup
          ? "signup_with"
          : "signin_with",
        shape: "rectangular",
          ...
          {!isOtp ? (
            <>
              ...
              <div className="flex flex-col gap-3 mb-6">
                <div id="google-signin-btn" className="w-full flex justify-center h-[46px] overflow-hidden rounded-2xl bg-white"></div>
              </div>
